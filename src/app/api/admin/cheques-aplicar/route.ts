...
interface Cheque {
  id: string; numero: string; fecha_cobro: string | null;
...
export async function POST() {
  ...
  const { data: cheques, error: chErr } = await (sb.from("cheques") as any)
    .select("id, numero, fecha_cobro, tipo_mercaderia, monto_uyu, monto_usd");
  ...
  const idsToClassify: { id: string; categoria: string }[] = [];
  ...
  for (const ch of (cheques as Cheque[])) {
    ...
    if (match && match.clasificado !== "Si") {
      idsToClassify.push({ id: match.id, categoria: ch.tipo_mercaderia ?? "Mercadería" });
    }
  }
  ...
  // Classify each matched movement as negocio + tipo_mercaderia
  let clasificados = 0;
  for (const { id, categoria } of idsToClassify) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const { error } = await (sb.from("bank_statements") as any)
      .update({ tipo: "negocio", categoria_negocio: categoria, clasificado: "Si" })
      .eq("id", id);
